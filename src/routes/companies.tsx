...
import {
  ...
  getDriversByCompany, getVehiclesByCompany, getBillsByCompany,
  getCompanyOutstanding, savePayment, getPaymentsByCompany,
  type Company, type Bill, type Payment,
} from "../lib/store";
import { Plus, Search, Pencil, Trash2, X, Building2, Users, Truck, Receipt, CreditCard, TrendingUp } from "lucide-react";
...
function CompaniesPage() {
  ...
  const [form, setForm] = useState({ name: "", contactDetails: "", address: "" });
  const [payBillId, setPayBillId] = useState<string | null>(null);
  const [payAmount, setPayAmount] = useState("");

  const filtered = companies.filter((c) => c.name.toLowerCase().includes(search.toLowerCase()));

  const resetForm = () => { setForm({ name: "", contactDetails: "", address: "" }); setEditingId(null); setShowForm(false); };

  const handleSave = () => {
    if (!form.name.trim()) return;
    if (editingId) updateCompany(editingId, form);
    else saveCompany(form);
    setCompanies(getCompanies());
    resetForm();
  };

  const handleEdit = (c: Company) => {
    setForm({ name: c.name, contactDetails: c.contactDetails, address: c.address });
    setEditingId(c.id);
    setShowForm(true);
  };
  ...
  const handlePayment = (billId: string, companyId: string) => {
    ...
    setPayBillId(null);
    setPayAmount("");
    setCompanies(getCompanies()); // refresh
  };
    ...
    <AppLayout>
      <div className="space-y-4">
        ...
        <div className="relative">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search companies..." className="w-full rounded-md border border-input bg-secondary pl-9 pr-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
        </div>
        ...
        {showForm && (
          <div className="stat-card space-y-3">
            ...
            <div><label className="field-label">Company Name *</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Enter company name" className="w-full rounded-md border border-input bg-secondary px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" /></div>
            <div><label className="field-label">Contact Details</label><input value={form.contactDetails} onChange={(e) => setForm({ ...form, contactDetails: e.target.value })} placeholder="Phone / Email" className="w-full rounded-md border border-input bg-secondary px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" /></div>
            <div><label className="field-label">Address</label><input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder="Address" className="w-full rounded-md border border-input bg-secondary px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" /></div>
        ...
        <div className="space-y-2">
          {filtered.map((c) => {
            ...
            const drivers = isExpanded ? getDriversByCompany(c.id) : [];
            const vhcls = isExpanded ? getVehiclesByCompany(c.id) : [];
              ...
              <div key={c.id} className="stat-card">
                <div className="flex items-start justify-between">
                  <div className="cursor-pointer flex-1" onClick={() => setExpandedId(isExpanded ? null : c.id)}>
                    ...
                    {c.contactDetails && <p className="text-xs text-muted-foreground mt-0.5">{c.contactDetails}</p>}
                ...
                {isExpanded && (
                  <div className="mt-3 border-t border-border pt-3 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <p className="text-xs font-medium text-muted-foreground flex items-center gap-1 mb-1"><Users className="h-3 w-3" /> Drivers ({drivers.length})</p>
                        {drivers.map((d) => <p key={d.id} className="text-xs text-foreground">{d.name} {d.phone && `· ${d.phone}`}</p>)}
                        {drivers.length === 0 && <p className="text-xs text-muted-foreground">No drivers</p>}
                      </div>
                      <div>
                        <p className="text-xs font-medium text-muted-foreground flex items-center gap-1 mb-1"><Truck className="h-3 w-3" /> Vehicles ({vhcls.length})</p>
                        {vhcls.map((v) => <p key={v.id} className="text-xs text-foreground">{v.number} ({v.capacity})</p>)}
                        {vhcls.length === 0 && <p className="text-xs text-muted-foreground">No vehicles</p>}
                      </div>
                    </div>

                    {/* Bills */}
                    <div>
                      <p className="text-xs font-medium text-muted-foreground flex items-center gap-1 mb-2"><Receipt className="h-3 w-3" /> Transactions ({bills.length})</p>
